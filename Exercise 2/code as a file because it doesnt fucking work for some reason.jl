...
m_p1f5 = f_p_p1
m_p2f5 = f_p_p2
m_f5d  = [sum(f5(p1, p2, d) * m_p1f5[p1] * m_p2f5[p2] for (p1, p2) in Iterators.product(1:N, 1:N)) for d in D]
f_p_d  = m_f5d
# setting y = 1
# backward pass
m_f6d  = f6.(D)
b_p_d  = m_f5d .* m_f6d # backward pass probs
m_df5  = b_p_d ./ m_f5d .* Int.(m_f5d .> 0)
